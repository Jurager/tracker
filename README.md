# Laravel Authenticate Tracker

#### Tracks and manages sessions, Passport Tokens and Sanctum Tokens in Laravel.

This package allows you to track separately each login (session or token), attaching information by User-Agent and saving the IP address.

Using a supported provider or creating your own custom providers, you can collect even more information with
an IP address lookup to get, for example geolocation.
...
- This package has been tested with **Laravel 5.8, 6.x and 7.x, 8.x.**

- Works with all the session drivers supported by Laravel, except cookie and array driver.

- For tracking tokens, it supports official **Passport (>= 7.5)** and **Sanctum (>= 2.0)** packages.

- For Passport with multiple user providers, use the
[`sfelix-martins/passport-multiauth`](https://github.com/sfelix-martins/passport-multiauth) package.
...
Publish the configuration file (`config/tracker.php`) with:
...
Before running migrations, you can change the name of the table that will be used to save the logins
with the `table_name` option of the configuration file.
...
In order to track the logins of your app's users, add the `Jurager\Tracker\Traits\Tracking` trait
...
by the `Jurager\Tracker\Traits\AuthenticatesWithTracking`

This overrides the `sendLoginResponse` method by removing the session regeneration.
But don't worry, there's no security issue here.
Instead, this package does the session regeneration in an event
listener on the login event.
...
There are supported two of the most popular parsers:
- [WhichBrowser](https://github.com/WhichBrowser/Parser-PHP)
- [Agent](https://github.com/jenssegers/agent)

Before using this package, choose a supported parser, install it and indicate in the configuration
to use.

### Configure the user provider

Package comes with a modified Eloquent User Provider that retrieve remembered users from the logins table instead of the users table.

In your `config/auth.php` configuration file, use the `tracked` driver in the user providers list for the users you want to track:
...
Dispatch the `Jurager\Tracker\Events\PersonalAccessTokenCreated` event passing the personal access token
...
The `Tracking` trait provided by this package surcharge your users models with methods to list their logins and to
...
On a new login, you can listen to the event `Jurager\Tracker\Events\Login`.
It receives a `RequestContext` object containing all the information collected on the request, accessible on the event
...
`Jurager\Tracker\Interfaces\IpProvider` interface and use the
`Jurager\Tracker\Traits\MakesApiCalls` trait.
