...
class IpProviderFactory
{
    ...
    public static function build($name)
    {
        if (self::ipLookupEnabled()) {
            ...
            $customProviders = config('tracker.ip_lookup.custom_providers');
    ...
    {
        return config('tracker.ip_lookup.provider') &&
            App::environment(config('tracker.ip_lookup.environments'));
    }
}
